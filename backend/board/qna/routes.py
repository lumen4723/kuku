...
async def get_article(slug: str, session: Session = Depends(utils.database.get_db)):
    return database.list_article_by_slug(session,slug = slug,all = True).map_err(throwMsg).unwrap()

# get article by slug start_page
@router.get("/list/slug/{slug}/{start_page}")
async def get_article(
    slug: str, start_page: int, limit: int = 20, session: Session = Depends(utils.database.get_db)
):
    return (
...
@router.delete(
    "/article/{article_id}", dependencies=[Depends(cookie)], status_code=status.HTTP_204_NO_CONTENT
)
async def delete_article_by_id(
    article_id: int, session: Session = Depends(utils.database.get_db), session_data: SessionData = Depends(verifier)
):
    return database.delete_article(article_id, session_data.uid, session).map_err(throwMsg).unwrap()
...
@router.post(
    "/article/{article_id}/like", dependencies=[Depends(cookie)], status_code=status.HTTP_201_CREATED
)
async def like_article_by_id(
    article_id: int, session: Session = Depends(utils.database.get_db), session_data: SessionData = Depends(verifier)
):
    return create_qna_like(article_id, session_data.uid, session).map_err(throwMsg).unwrap()

@router.put(
    "/article/{article_id}/dislike", dependencies=[Depends(cookie)], status_code=status.HTTP_201_CREATED
)
async def dislike_article_by_id(
    article_id: int, session: Session = Depends(utils.database.get_db), session_data: SessionData = Depends(verifier)
):
    return cancel_qna_like(article_id, session_data.uid, session).map_err(throwMsg).unwrap()
