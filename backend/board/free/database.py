...
from ...user import database as UserDatabase


class BoardFree(SQLModel, table=True):
    article_id: Optional[int] = Field(default=None, primary_key=True)
    title: str
    content: str

    userid: int = Field(default=None, foreign_key="user.uid")
    userRel: "UserDatabase.User" = Relationship(back_populates="free")
...
def create_article(object_in: BoardFree, uid: int, db: Session) -> Result:
    try:
        article = BoardFree.from_orm(object_in)
        article.userid = uid
        db.add(article)
        db.commit()
        db.refresh(article)
...
def get_all_article_free(db: Session):
    return db.query(BoardFree).all()


# get ariticle
def get_article(start_page: int, db: Session):
    try:
        article_ct = db.query(BoardFree).count()
        start = 0 + (start_page - 1) * 10
        articles = db.query(BoardFree).offset(start).limit(10).all()
...
def get_article_by_id(article_id: int, db: Session):
    try:
        article = db.query(BoardFree).filter_by(article_id=article_id).first()
        return Ok(article)
    except Exception as e:
        err_msg = str(e).lower()
        if "background" in err_msg:
